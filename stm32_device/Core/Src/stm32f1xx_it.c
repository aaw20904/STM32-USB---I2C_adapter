...
extern volatile   uint8_t i2cRxSlaveBuffer[260];
extern volatile   uint8_t i2cTxSlaveBuffer[260];
extern volatile  uint16_t slaveI2cBufferLimit;

extern volatile uint8_t i2cRxMasterBuffer[1026];
extern volatile   uint8_t i2cTxMasterBuffer[1026];
extern volatile wrp_i2c_master_header i2cMasterHeader;
extern volatile uint32_t adapterSemaphoreI2C;
extern volatile uint32_t adapterSemaphoreSPI;
extern volatile wrp_spi_master_header spiMasterHeader;
extern  volatile  uint8_t spiRxMasterBuffer[1026];
extern  volatile  uint8_t spiTxMasterBuffer[1026];
...
void DMA1_Channel2_IRQHandler(void)
{
  /* USER CODE BEGIN DMA1_Channel2_IRQn 0 */
    if (DMA1->ISR & DMA_ISR_TCIF2) {
   	  ...
   	  //disable DMA channel
    spiMasterHeader.dmaRxChanel->CCR &= ~DMA_CCR_EN;
...
void DMA1_Channel3_IRQHandler(void)
{
  /* USER CODE BEGIN DMA1_Channel3_IRQn 0 */
     if (DMA1->ISR & DMA_ISR_TCIF3) {
       //disable DMA channel
      spiMasterHeader.dmaTxChanel->CCR &= ~DMA_CCR_EN;
      //clear DMA flag
